...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        let baseUrl = "https://local-channel-gateway-dev.dev.truemoney.com.kh/"
...
//            print("*** \(json) \(code) \(error)")
//        }
        TrueRequestAccessToken.url = baseUrl
        let req = TrueRequestAccessToken()
        req.setPath(endpoint)
        req.setHeader()
        req.setBody(param)
        req.setMethod("POST")
        
        req.send().dataTask(with: req.getRequest()) { (json, response, error) in
